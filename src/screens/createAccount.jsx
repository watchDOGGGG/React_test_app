...
export default function CreateAccount() {
	...
	const handleFileUpload = (event) => {
		...
		if (
			file &&
			!['jpeg', 'png', 'jpg', 'svg+xml'].includes(fileExtension)
		) {
			setErrorFile(
		...
		<div className='flex flex-col items-center justify-center mt-12'>
			...
			<form className='w-96 shadow-lg p-4 mt-10'>
				...
				<div className='flex flex-col mt-5 font-bold'>
					...
					{error?.filename && (
						<p className='text-red-600 text-sm font-bold'>{error?.filename}</p>
					)}
					{errorFile && (
						<p className='text-red-600 text-sm font-bold'>{errorFile}</p>
					)}
				</div>

				{err && <p className='text-red-600 text-sm font-bold'>{err?.message}</p>}
